...
export function AdminOperationalControls({ onMessage }: Props) {
  ...
  const [supportedVehicles, setSupportedVehicles] = useState<VehicleType[]>([
    "bike",
    "auto",
    "car",
  ]);
  ...
  const loadControls = useCallback(async () => {
    ...
    if (
      ...
      isMissingSupabaseObject(fraudResult.error)
    ) {
      setMigrationMissing(true);
      onMessage(
        "Operational controls need the latest Supabase migrations before service areas, pricing, and fraud review can be used.",
      );
      return;
    }
    ...
    if (areasResult.error || rulesResult.error || fraudResult.error) {
      onMessage(
        areasResult.error?.message ??
          ...
          fraudResult.error?.message ??
          "Operational controls need the latest Supabase migration.",
      );
      return;
    }
  ...
  async function createPricingRule() {
    ...
    if (
      [
        payload.base_fare,
        payload.company_commission_rate,
        payload.minimum_fare,
        payload.per_km_rate,
        payload.per_minute_rate,
  ...
  const previewSplit = useMemo(
    () => calculateFareBreakdown(200, Number(commissionRate) || 0.07),
    [commissionRate],
  );
    ...
    return (
      <Card className="rounded-[1.75rem] p-5">
        <CardHeader className="p-0">
          <CardTitle>Operational migration required</CardTitle>
          <CardDescription>
            Service areas, pricing rules, and fraud signals are not available in
        ...
        <div className="mt-4 grid gap-2 text-sm font-semibold text-muted-foreground">
          <p className="rounded-2xl bg-muted p-3">
            Apply 20260703110000_operational_and_product_foundation.sql.
          </p>
          <p className="rounded-2xl bg-muted p-3">
            Apply 20260706100000_operational_enforcement_and_fraud.sql.
          </p>
          <p className="rounded-2xl bg-muted p-3">
            Then run the Supabase schema reload command in SQL Editor.
          </p>
        </div>
      </Card>
    );
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(22rem,0.75fr)]">
      <div className="grid gap-5">
        <Card className="rounded-[1.5rem] p-4 sm:p-5">
          <CardHeader className="px-0 pt-0">
            <CardTitle className="flex items-center gap-2">
              <MapPinned className="size-5" /> Service areas
            </CardTitle>
            <CardDescription>
              Define where Taxiro can accept trips and which verified vehicles
              can serve that area.
            </CardDescription>
          </CardHeader>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field
              ...
              type="number"
              value={centerLat}
              onChange={setCenterLat}
            />
            <Field
              label="Center longitude"
              type="number"
              value={centerLng}
              onChange={setCenterLng}
            />
          </div>
          ...
          <Button
            ...
            onClick={() => void createServiceArea()}
          >
            Create service area
          </Button>
        </Card>
        ...
        <Card className="rounded-[1.5rem] p-4 sm:p-5">
          <CardHeader className="px-0 pt-0">
            <CardTitle className="flex items-center gap-2">
              <SlidersHorizontal className="size-5" /> Pricing rules
            </CardTitle>
            <CardDescription>
              Configure fares per area and vehicle. Bookings save the matched
              rule for audit.
            </CardDescription>
          </CardHeader>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="grid gap-1 text-sm font-bold">
              Service area
              <select
                className="h-11 rounded-xl border border-border bg-card px-3 outline-none focus:ring-2 focus:ring-ring"
                onChange={(event) => setRuleAreaId(event.target.value)}
                value={ruleAreaId}
              >
            ...
            <label className="grid gap-1 text-sm font-bold">
              Vehicle
              <select
                className="h-11 rounded-xl border border-border bg-card px-3 capitalize outline-none focus:ring-2 focus:ring-ring"
                onChange={(event) =>
                  setRuleVehicle(event.target.value as VehicleType)
                }
                value={ruleVehicle}
              >
                {vehicleTypes.map((vehicle) => (
            ...
            <div className="rounded-xl bg-muted p-3 text-sm">
              ...
              <p className="mt-1 text-muted-foreground">
                Taxiro {formatMoney(previewSplit.companyCommission)} / Rider{" "}
                {formatMoney(previewSplit.riderEarning)}
              </p>
            </div>
          </div>
          <Button
            className="mt-4 w-full rounded-xl"
            onClick={() => void createPricingRule()}
          >
            Create pricing rule
          </Button>
        </Card>
      ...
      <div className="grid gap-5">
        <Card className="rounded-[1.5rem] p-4 sm:p-5">
          <CardHeader className="px-0 pt-0">
            <CardTitle className="flex items-center gap-2">
              <ShieldAlert className="size-5" /> Fraud review
            </CardTitle>
            <CardDescription>
              Location jumps and suspicious tracking signals are queued here for
              admin review.
            </CardDescription>
          </CardHeader>
              ...
              fraudSignals.map((signal) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 rounded-full bg-card px-2 py-1 text-[10px] font-black">
                      {new Date(signal.created_at).toLocaleTimeString()}
                    </span>
                  </div>
                  <pre className="mt-3 max-h-28 overflow-auto rounded-xl bg-card p-3 text-[11px] text-muted-foreground">
                    {JSON.stringify(signal.evidence, null, 2)}
                  </pre>
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    <Button
                      onClick={() =>
                        void reviewFraudSignal(signal.id, "reviewing")
                      }
                      size="sm"
                      variant="outline"
                    >
                    ...
                    <Button
                      onClick={() =>
                        void reviewFraudSignal(signal.id, "dismissed")
                      }
                      size="sm"
                      variant="outline"
                    >
                    ...
                    <Button
                      onClick={() =>
                        void reviewFraudSignal(signal.id, "confirmed")
                      }
                      size="sm"
                    >
                      Confirm
            ...
            {pricingRules.slice(0, 8).map((rule) => (
              <div className="rounded-xl bg-muted p-3 text-sm" key={rule.id}>
                <p className="font-black">
                  {getVehicleLabel(rule.vehicle_type)} - Rs {rule.per_km_rate}
                  /km
                </p>
                <p className="text-xs text-muted-foreground">
                  Base {formatMoney(rule.base_fare)} / minimum{" "}
                  {formatMoney(rule.minimum_fare)} / commission{" "}
                  {(rule.company_commission_rate * 100).toFixed(1)}%
                </p>
              </div>
            ))}
            {!pricingRules.length ? (
