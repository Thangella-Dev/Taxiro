...
function readySignalLabel(ride: RideRequest, now: number) {
  if (ride.status !== "ready") {
    return new Date(ride.scheduled_time).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
  }
  ...
  const minutes = Math.max(
    0,
    Math.ceil((new Date(ride.ready_expires_at).getTime() - now) / 60_000),
  );
  return minutes <= 1
    ? "ready, under 1 min left"
    : `ready, ${minutes} min left`;
}
...
export function DemandSignals({
  ...
  return (
    <Card
      className={
        compact
          ? "min-w-0 max-w-full overflow-hidden border-secondary/50 bg-secondary/20 p-3"
          : "min-w-0 max-w-full overflow-hidden p-4"
      }
    >
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 text-sm">
        <div className="rounded-lg bg-secondary p-3 font-black">
          {ready.length} ready now
        </div>
        <div className="rounded-lg bg-muted p-3 font-black">
          ...
          visibleSignals.map((ride) => {
            ...
            return (
              <div
                className={
                  readyNow
                    ? "min-w-0 rounded-lg border border-secondary bg-secondary/70 p-3"
              ...
              >
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-3">
                    {readyNow ? (
                      <Radio className="mt-1 size-4 shrink-0 text-primary" />
                    ) : (
                      <MapPinned className="mt-1 size-4 shrink-0 text-primary" />
                    )}
                    <div className="min-w-0">
                      <p className="truncate text-sm font-black">
                        {ride.pickup_address.split(",")[0] || "Pickup area"}
                      </p>
                      <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
                        <Clock3 className="size-3" />{" "}
                        {readySignalLabel(ride, now)}
                      </p>
